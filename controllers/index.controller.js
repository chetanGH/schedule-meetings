...
module.exports.createNewMeeting = async(req,res)=>{
    try {
        ...
        if(membersArr.length > 1){
            ...
            return res.status(405).send({success:false,message:'At least two participants are required to schedule meeting.'})
        }
    } catch (error) {
        console.log(error);
        return res.status(500).send({success:false,message:'Internal server error.'})
...
module.exports.basicMeeting = async(req,res)=>{
    try {
        ...
        if(members.length > 1){
            ...
            return res.status(405).send({success:false,message:"Atleast 2 members are required to create meeting."})
        }
    } catch (error) {
        console.log(error);
        return res.status(500).send({success:false,message:'Internal server error.'})
